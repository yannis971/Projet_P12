...
class SalesContactViewTest(TestCase, TestInterface):
    ...
    def test_change_salescontact(
        self, username, password, new_password, pk, status_code
    ):
        self.login(username, password)
...
class SupportContactViewTest(TestCase, TestInterface):
    ...
    def test_change_supportcontact(
        self, username, password, new_password, pk, status_code
    ):
        self.login(username, password)
        ...
        data = {
            "user": {
                "username": support_contact.user.username,
...
class StaffContactViewTest(TestCase, TestInterface):
    ...
    def test_change_staffcontact(
        ...
        data = {
            "user": {"username": staff_contact.user.username, "password": new_password}
        }
        response = self.client.put(url, data, content_type="application/json")
        self.assertEqual(response.status_code, status_code)
...
class ClientViewTest(TestCase, TestInterface):
    ...
    def test_change_client(
        self, username, password, pk, new_phone, new_mobile, status_code
    ):
        self.login(username, password)
        url = f"/clients/{pk}/"
...
class ContractViewTest(TestCase, TestInterface):
    ...
    def test_change_contract(
        self, username, password, pk, new_status, new_amount, new_date_iso, status_code
    ):
        self.login(username, password)
        url = f"/contracts/{pk}/"
        ...
        if status_code == status.HTTP_200_OK:
            ...
            self.assertEqual(
                datetime.fromisoformat(response.data["payment_due"]),
                datetime.fromisoformat(new_date_iso),
            )
