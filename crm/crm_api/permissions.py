...
class ContractPermission(permissions.BasePermission):
    ...
    def change_contract_status_only(self, request, obj):
        ...
        try:
            ...
            pass
        return bool(amount == obj.amount
                    and request.data["payment_due"] == obj.payment_due.strftime('%Y-%m-%dT%H:%M:%SZ')
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if view.action == 'update' or view.action == 'partial_update':
            user_has_perm = bool(self.change_contract_status_only(request, obj) or request.user.has_perm(self._perms[view.action]))
        else:
